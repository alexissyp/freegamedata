...
import { getAndroidGames } from "context/platforms";
import { DefaultGames } from "../../../../constants";
import { useLocation } from "wouter";
import GameDisplay from "../../gameDisplay";

const Android = ({ gameTitle }) => {
  const [androidGames, setAndroidGames] = useState(DefaultGames);
  const [loading, setLoading] = useState(true);
  const [, setLocation] = useLocation();

  useEffect(() => {
    if (loading) {
      getAndroidGames().then((response) => {
        setAndroidGames(response);
        setLoading(false);
      });
    }
  }, [loading]);
  ...
  if (gameTitle)
    return <GameDisplay allGames={androidGames} gameTitle={gameTitle} />;
